...
/*   Updated: 2025/02/12 14:06:31 by bepoisso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	mutex_error(int status, t_mutex_code code)
{
	...
	if (status == EINVAL && (code == lock || code == unlock))
		...
		ft_perror("Error Mutex\nA deadlock wold occur if the thread \
			blocked waiting for mutex.\n");
	else if (status == EPERM)
		ft_perror ("Error Mutex\nThe current thread dose not hold a \
			lock on mutex.\n");
	else if (status == ENOMEM)
		ft_perror("Error Mutex\nThe process cannot allocate enough \
			memoty to create another mutex.\n");
